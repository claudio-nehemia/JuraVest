...
export default function Show({ auth, receiverId, users }: Props) {
    ...
    const currentReceiver = users.find((user) => user.id === receiverId);
            ...
            <div className="flex h-[calc(100vh-64px)]">
                ...
                <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
                    <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-500 to-purple-600">
                        ...
                        <p className="text-blue-100 text-sm">Pilih kontak untuk memulai chat</p>
                    </div>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className={`text-sm font-medium truncate ${
                                                    user.id === receiverId ? 'text-blue-700' : 'text-gray-900'
                                                }`}>
                                                    {user.name}
                                                </p>
                                                <p className="text-xs text-gray-500">
                                                    {user.id === receiverId ? 'On Chat' : 'Ketuk untuk memulai chat'}
                                                </p>
                                            </div>
                                            {user.id === receiverId && (
                ...
                <div className="flex-1 flex flex-col bg-white">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50 to-white">
                        {messages.length === 0 ? (
                            ...
                            messages.map((msg, i) => (
                                <div key={i} className={`flex ${msg.sender_id === auth.user.id ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${
                                        ...
                                        <p className={`text-xs mt-1 ${
                                            msg.sender_id === auth.user.id ? 'text-blue-100' : 'text-gray-500'
                                        }`}>
