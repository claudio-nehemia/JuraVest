...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Data Wirausaha Baru',
        href: '/admin/wirausaha/newIndex'
    }
];

export default function NewIndex({wirausahas}: Props) {
    ...
    return (
       <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Data Wirausaha Baru" />
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="rounded-lg border">
                    <div className="flex items-center justify-between p-4">
                        <h1 className="text-2xl font-bold">Data Wirausaha Baru</h1>
                        <div className='flex items-center justify-between p-4'>
                    ...
                    <Table>
                        ...
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className='text-center'>Rencana Mulai</TableHead>
                                <TableHead className="text-center">Jenis Usaha</TableHead>
                                <TableHead className="text-center">Rencana Lokasi</TableHead>
                                <TableHead className='text-center'>Perkiraan Dana</TableHead>
                                <TableHead className='text-center'>Alokasi Dana</TableHead>
                        ...
                        <TableBody>
                            {wirausahas && wirausahas.length > 0 ? (
                                wirausahas.map((wirausaha) => (
                                    ...
                                    <TableCell className='text-center'>{wirausaha.usaha_baru?.rencana_mulai_usaha}</TableCell>
                                    <TableCell className='text-center'>{wirausaha.jenis_usaha?.jenis_usaha}</TableCell>
                                    <TableCell className='text-center'>{wirausaha.usaha_baru?.rencana_lokasi_operasional}</TableCell>
                                    <TableCell className='text-center'>{wirausaha.usaha_baru?.perkiraan_dana}</TableCell>
                                    <TableCell className='text-center'>{wirausaha.usaha_baru?.alokasi_dana}</TableCell>
                                    <TableCell className='text-center'>
                                    <TooltipProvider>
                                    <div className="flex justify-center space-x-2">
                                        <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Link href={`/admin/wirausaha/newEdit/${wirausaha.id}/edit`}>
